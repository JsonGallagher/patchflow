...
void DisplaceNode::ensureFBO (juce::OpenGLContext& gl, int width, int height)
{
...
void DisplaceNode::compileShader (juce::OpenGLContext& gl)
{
    if (shaderProgram_ != 0)
        return;
    ...
    const juce::String fragmentShaderSource =
        ...
        "    if (u_wrap == 1)\n"
        "        return texture2D(u_source, fract(uv));\n"
        "\n"
        ...
        "    if (u_mode == 0)\n"
        "    {\n"
        "        float luma = dot(disp.rgb, vec3(0.299, 0.587, 0.114)) - 0.5;\n"
        "        offset = rot * vec2(luma, 0.0);\n"
        "    }\n"
        "    else\n"
        "    {\n"
        "        offset = rot * ((disp.rg * 2.0) - 1.0);\n"
        "    }\n"
        "\n"
        "    vec2 uv = v_uv + offset * u_amount;\n"
        "    gl_FragColor = sampleSource(uv);\n"
        "}\n";
    ...
    {
        shaderError_ = true;
        DBG ("DisplaceNode vertex shader compile error:\n" + errorLog);
        return;
    }
    ...
    if (fs == 0)
    {
        ...
        shaderError_ = true;
        DBG ("DisplaceNode fragment shader compile error:\n" + errorLog);
        return;
    }
    ...
    if (! linked)
    {
        ...
        shaderProgram_ = 0;
        shaderError_ = true;
        DBG ("DisplaceNode shader link error:\n" + linkLog);
        return;
    }
...
void DisplaceNode::ensureFallbackTextures()
{
    if (fallbackSourceTexture_ == 0)
    {
        juce::gl::glGenTextures (1, &fallbackSourceTexture_);
        juce::gl::glBindTexture (juce::gl::GL_TEXTURE_2D, fallbackSourceTexture_);
    ...
    if (fallbackDisplacementTexture_ == 0)
    {
        juce::gl::glGenTextures (1, &fallbackDisplacementTexture_);
        juce::gl::glBindTexture (juce::gl::GL_TEXTURE_2D, fallbackDisplacementTexture_);

        const juce::uint8 neutralPixel[4] = { 128, 128, 0, 255 };
        juce::gl::glTexImage2D (juce::gl::GL_TEXTURE_2D, 0, juce::gl::GL_RGBA8,
                                1, 1, 0,
                                juce::gl::GL_RGBA, juce::gl::GL_UNSIGNED_BYTE,
                                neutralPixel);
...
void DisplaceNode::renderFrame (juce::OpenGLContext& gl)
{
    ...
    else
    {
        ...
        float amount = getParamAsFloat ("amount", 0.03f);
        float rotationDeg = getParamAsFloat ("rotationDeg", 0.0f);

        if (isInputConnected (2))
            amount *= juce::jlimit (0.0f, 2.0f, getConnectedVisualValue (2) * 2.0f);

        if (isInputConnected (3))
            rotationDeg += getConnectedVisualValue (3) * 180.0f;

        amount = juce::jlimit (0.0f, 1.0f, amount);

        if (auto amountLoc = loc ("u_amount"); amountLoc >= 0)
            gl.extensions.glUniform1f (amountLoc, amount);

        if (auto rotateLoc = loc ("u_rotation"); rotateLoc >= 0)
            gl.extensions.glUniform1f (rotateLoc, juce::degreesToRadians (rotationDeg));

        if (auto modeLoc = loc ("u_mode"); modeLoc >= 0)
            gl.extensions.glUniform1i (modeLoc, getParamAsInt ("mode", 1));
        ...
        juce::gl::glBindTexture (juce::gl::GL_TEXTURE_2D,
                                 isInputConnected (0) ? getConnectedTexture (0) : fallbackSourceTexture_);
        if (auto sourceLoc = loc ("u_source"); sourceLoc >= 0)
            gl.extensions.glUniform1i (sourceLoc, 0);

        juce::gl::glActiveTexture (juce::gl::GL_TEXTURE1);
        juce::gl::glBindTexture (juce::gl::GL_TEXTURE_2D,
                                 isInputConnected (1) ? getConnectedTexture (1) : fallbackDisplacementTexture_);
        if (auto dispLoc = loc ("u_displacement"); dispLoc >= 0)
            gl.extensions.glUniform1i (dispLoc, 1);
