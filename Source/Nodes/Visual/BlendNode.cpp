...
void BlendNode::ensureFBO (juce::OpenGLContext& gl, int width, int height)
{
...
void BlendNode::compileShader (juce::OpenGLContext& gl)
{
    if (shaderProgram_ != 0)
        return;
    ...
    const juce::String fragmentShaderSource =
        ...
        "    vec3 op = b.rgb;\n"
        "    if (u_mode == 1) op = a.rgb + b.rgb;\n"
        "    else if (u_mode == 2) op = a.rgb * b.rgb;\n"
        "    else if (u_mode == 3) op = 1.0 - (1.0 - a.rgb) * (1.0 - b.rgb);\n"
        "    else if (u_mode == 4) op = abs(a.rgb - b.rgb);\n"
        "\n"
        "    vec3 outRgb = (u_mode == 0) ? mix(a.rgb, b.rgb, u_mix) : mix(a.rgb, clamp(op, 0.0, 1.0), u_mix);\n"
        "    float outA = mix(a.a, b.a, u_mix);\n"
    ...
    {
        shaderError_ = true;
        DBG ("BlendNode vertex shader compile error:\n" + errorLog);
        return;
    }
    ...
    if (fs == 0)
    {
        ...
        shaderError_ = true;
        DBG ("BlendNode fragment shader compile error:\n" + errorLog);
        return;
    }
    ...
    if (! linked)
    {
        ...
        shaderProgram_ = 0;
        shaderError_ = true;
        DBG ("BlendNode shader link error:\n" + linkLog);
        return;
    }
...
void BlendNode::ensureFallbackTexture()
{
...
void BlendNode::renderFrame (juce::OpenGLContext& gl)
{
    ...
    else
    {
        ...
        auto mixValue = getParamAsFloat ("mix", 0.5f);
        if (isInputConnected (2))
            mixValue += getConnectedVisualValue (2) - 0.5f;
        mixValue = juce::jlimit (0.0f, 1.0f, mixValue);

        if (auto mixLoc = loc ("u_mix"); mixLoc >= 0)
            gl.extensions.glUniform1f (mixLoc, mixValue);

        if (auto modeLoc = loc ("u_mode"); modeLoc >= 0)
            gl.extensions.glUniform1i (modeLoc, getParamAsInt ("mode", 0));
        ...
        if (auto texALoc = loc ("u_texA"); texALoc >= 0)
            gl.extensions.glUniform1i (texALoc, 0);

        juce::gl::glActiveTexture (juce::gl::GL_TEXTURE1);
        juce::gl::glBindTexture (juce::gl::GL_TEXTURE_2D,
                                 isInputConnected (1) ? getConnectedTexture (1) : fallbackTexture_);
        if (auto texBLoc = loc ("u_texB"); texBLoc >= 0)
            gl.extensions.glUniform1i (texBLoc, 1);
